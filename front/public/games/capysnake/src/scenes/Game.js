...
// Valeurs et couleurs des fruits bonus (segments ajoutés, couleur)
const BONUS_FRUITS = [
    { value: 2, color: 0x00ccff },
    { value: 4, color: 0xff8800 },
    { value: 6, color: 0xffcc00 }
];
...
export class Game extends Phaser.Scene {
    constructor() {
        ...
        this.foodStem = null;
        this.bonusFood = null;
        this.bonusFoodStem = null;
        this.bonusFoodValue = 0;
    ...
    create() {
        ...
        for (let i = 0; i < SNAKE_INITIAL_LENGTH; i++) {
            ...
            const segment = this.add.image(x, y, 'capy')
        ...
        this.bestText = this.add.text(this.worldWidth - 16, hudY, `Best: ${this.highScore}`, {
            ...
            color: '#ccffaa', stroke: '#000000', strokeThickness: 3
        }).setOrigin(1, 0.5).setDepth(10);

        // Indicateur d'accélération (coin haut gauche du HUD, droite du score)
        this.boostText = this.add.text(cx, hudY, 'BOOST ⚡', {
            fontFamily: 'Arial Black', fontSize: 20,
            color: '#ffee00', stroke: '#000000', strokeThickness: 3
        }).setOrigin(0.5, 0.5).setDepth(10).setVisible(false);

        // === TUTORIAL ===
        const playCenterY = this.playAreaY + (this.worldHeight - this.playAreaY) / 2;
        this.tutorialText = this.add.text(cx, playCenterY - 60,
            'Flèches / swipe pour jouer\nSHIFT pour accélérer', {
    ...
    update(time) {
        ...
        // SHIFT = boost 2× (ne descend pas en dessous de 50ms)
        const boosting = this.shiftKey.isDown;
        const activeSpeed = boosting
            ? Math.max(50, Math.floor(this.snakeSpeed / 2))
            : this.snakeSpeed;
        this.boostText.setVisible(boosting);

        if (time >= this.lastMoveTime + activeSpeed) {
            this.moveSnake();
    ...
    moveSnake() {
        ...
        const eatingFood  = this.food  && newX === this.food.x  && newY === this.food.y;
        const eatingBonus = this.bonusFood && newX === this.bonusFood.x && newY === this.bonusFood.y;

        this.directionHistory.unshift(this.direction);

        // Ancienne tête → segment corps
        ...
        // Gestion de la queue
        if (!eatingFood && !eatingBonus && this.pendingGrowth === 0) {
            const tail = this.snake.pop();
            tail.destroy();
            this.directionHistory.pop();
        } else if (this.pendingGrowth > 0 && !eatingFood && !eatingBonus) {
            // Croissance progressive du fruit bonus
            this.pendingGrowth--;
        }

        if (eatingFood) {
            this.spawnFlash(this.food.x, this.food.y, 0xffff00);
            this.food.destroy();
            ...
            // 30% de chance de faire apparaître un fruit bonus
            if (!this.bonusFood && Phaser.Math.Between(1, 10) <= 3) {
                this.spawnBonusFood();
            }
        }

        if (eatingBonus) {
            this.spawnFlash(this.bonusFood.x, this.bonusFood.y, 0xffffff);
            this.bonusFood.destroy();
            if (this.bonusFoodStem) this.bonusFoodStem.destroy();
            this.bonusFood = null;
            this.bonusFoodStem = null;

            // pendingGrowth -1 car ce coup-ci on n'a pas retiré la queue
            this.pendingGrowth += this.bonusFoodValue - 1;
            this.score += this.bonusFoodValue * 10;
            this.scoreText.setText(String(this.score));
            this.bonusFoodValue = 0;
        }

        // Orientation de tous les segments
    ...
    spawnFlash(x, y, color) {
        const flash = this.add.ellipse(x, y, this.gridSize * 1.4, this.gridSize * 1.4, color, 0.85);
        this.tweens.add({
            targets: flash,
    ...
    spawnFood() {
        ...
        if (!position) { this.gameOver(); return; }
    ...
    spawnBonusFood() {
        const occupied = this.getOccupiedSet();
        if (this.food) occupied.add(this.getGridKey(this.food.x, this.food.y));
        const position = this.getFreePosition(occupied);
        if (!position) return;

        const { x, y } = position;
        const bonus = BONUS_FRUITS[Phaser.Math.Between(0, BONUS_FRUITS.length - 1)];
        this.bonusFoodValue = bonus.value;

        const r = Math.floor(this.gridSize * 0.42);
        this.bonusFood = this.add.ellipse(x, y, r * 2, r * 2, bonus.color);
        this.bonusFoodStem = this.add.rectangle(x, y - r - 3, 2, 7, FOOD_STEM_COLOR);

        // Pulse pour le rendre visible
        this.tweens.add({
    ...
    applyHeadOrientation(segment, direction) {
        switch (direction) {
            case 'right': segment.setFlipX(true).setAngle(0);    break;
            case 'left':  segment.setFlipX(false).setAngle(0);   break;
            case 'up':    segment.setFlipX(true).setAngle(-90);  break;
            case 'down':  segment.setFlipX(true).setAngle(90);   break;
        }
    }
    ...
    gameOver() {
        ...
        if (this.bonusFood) { this.bonusFood.destroy();  this.bonusFood = null; }
        if (this.bonusFoodStem) { this.bonusFoodStem.destroy(); this.bonusFoodStem = null; }
    ...
    getFreePosition(occupied) {
        ...
        for (let gx = 0; gx < this.gridWidth; gx++) {
            for (let gy = 0; gy < this.gridHeight; gy++) {
                ...
                if (!occupied.has(this.getGridKey(x, y))) free.push({ x, y });
            }
        }
